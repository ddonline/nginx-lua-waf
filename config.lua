...
1、nginx配置：在http级别添加以下内容
    #x-waf配置
    lua_package_path "/opt/web_engine/nginx/conf/x-waf/?.lua;";
    lua_shared_dict limit 100m;
    #lua_shared_dict badGuys 100m;
    #开启lua代码缓存功能
    lua_code_cache on;
    lua_regex_cache_max_entries 4096;
    init_by_lua_file   /opt/web_engine/nginx/conf/x-waf/init.lua;
    access_by_lua_file /opt/web_engine/nginx/conf/x-waf/access.lua;
2、vhost配置：server_name第一个名称为规则主机限制时的关键词
    server_name  api app.dong.com;
3、规则格式：
...
7、性能：
   ab -n 10000 -c 50 http://192.168.158.139/index.html
   关闭waf  Requests per second:    10839.05 [#/sec] (mean)
   打开waf  Requests per second:    5694.25 [#/sec] (mean)
]]
...
local _M = {
    ...
    -- 规则文件存放目录 结尾不带/
    config_rule_dir = "/opt/web_engine/nginx/conf/x-waf/rules",
